...
void Control::hexapod_mode_cb(const std_msgs::Int32ConstPtr &mode)
{
  if (mode->data != robot_mode)
  {
    ...
    if (mode->data == crab_mode && robot_mode == climb2wall_mode) //此时置模式为crab_mode时，直接改变不需执行变换程序，初始化关节角（爬墙后的关节角）
    {
...
void Control::feedDrivers(const int &cycle_period_, std::vector<int> &cycle_leg_number_)
{
  ...
  if (std::abs(cmd_vel_.linear.x) > 0.001 || std::abs(cmd_vel_.linear.y) > 0.001 || std::abs(cmd_vel_.angular.z) > 0.001)
  {
    ...
    {
      if (gaitCnt == 1)  //重新接受第二步时的歩幅
      {
        ROS_INFO("Please publish the 2nd speed.");
        while (std::abs(cmd_vel_.linear.x) < 0.001 && std::abs(cmd_vel_.linear.y) < 0.001 && std::abs(cmd_vel_.angular.z) < 0.001)
        {
          ros::Duration(0.05).sleep();
        }
        ROS_INFO("2nd speed received");
        cmd_vel_step.linear.x = cmd_vel_.linear.x;
        cmd_vel_step.linear.y = cmd_vel_.linear.y;
        cmd_vel_step.angular.z = cmd_vel_.angular.z;
      }
      ...
      //给定六足的初始位姿，用于ik计算
      hexapod_msgs::FeetPositions initFeet;  
      legs2position(legs_, initFeet);
      ...
      for (int i = 0; i < maxpoints_; i++)
      {
        ...
        {
          hexapod_ik.calculateIK(initFeet, feet_, &legs_); //蜘蛛形态ik计算
        }
        else
        {
          crab_ik.calculateIK(initFeet, feet_, &legs_); //螃蟹形态ik计算
        }
...
void Control::legs2position(const hexapod_msgs::LegsJoints &legs, hexapod_msgs::FeetPositions &feet_position)
{
  for (int leg_index = 0; leg_index < 6; leg_index++)
  {
    feet_position.foot[leg_index].position.x = (COXA_LENGTH + FEMUR_LENGTH * cos(legs.leg[leg_index].femur) + TIBIA_LENGTH * sin(legs.leg[leg_index].femur + legs.leg[leg_index].tibia) + TARSUS_LENGTH * sin(legs.leg[leg_index].femur + legs.leg[leg_index].tibia + legs.leg[leg_index].tarsus)) * cos(INIT_COXA_ANGLE[leg_index] + legs.leg[leg_index].coxa);
    feet_position.foot[leg_index].position.y = (COXA_LENGTH + FEMUR_LENGTH * cos(legs.leg[leg_index].femur) + TIBIA_LENGTH * sin(legs.leg[leg_index].femur + legs.leg[leg_index].tibia) + TARSUS_LENGTH * sin(legs.leg[leg_index].femur + legs.leg[leg_index].tibia + legs.leg[leg_index].tarsus)) * sin(INIT_COXA_ANGLE[leg_index] + legs.leg[leg_index].coxa);
    feet_position.foot[leg_index].position.z = -(FEMUR_LENGTH * sin(legs.leg[leg_index].femur) - TIBIA_LENGTH * cos(legs.leg[leg_index].femur + legs.leg[leg_index].tibia) - TARSUS_LENGTH * cos(legs.leg[leg_index].femur + legs.leg[leg_index].tibia + legs.leg[leg_index].tarsus));
  }
}
...
//将误差表格设为update表，下一次执行时将按此次更新的表格进行机械误差补偿，并保存相应文件
void Control::saveMeclErrBalnRate_cb(const std_msgs::StringConstPtr &msg)
{
  ...
  tm *t = gmtime(&tt);
  std::string filename;
  filename = "/home/sun/hexapod_service_ws/src/hexapod_wall_control/params/meclErrBalnRate/meclErrBalnRate";
