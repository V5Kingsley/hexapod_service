...
Hexapod_IK::Hexapod_IK()
{
  ros::param::get("HEXAPOD_INIT_FOOT_POS_X", HEXAPOD_INIT_FOOT_POS_X);
  ros::param::get("HEXAPOD_INIT_FOOT_POS_Y", HEXAPOD_INIT_FOOT_POS_Y);
  ros::param::get("HEXAPOD_INIT_FOOT_POS_Z", HEXAPOD_INIT_FOOT_POS_Z);
}


void Hexapod_IK::calculateIK(const hexapod_msgs::FeetPositions initFeet, const hexapod_msgs::FeetPositions &feet, hexapod_msgs::LegsJoints *legs)
{
  for (int leg_index = 0; leg_index < NUMBER_OF_LEGS; leg_index++)
  {
    //机体旋转角度，angular.z
    Trig A = getSinCos(feet.foot[leg_index].orientation.yaw);
    //足端到机体中心距离
    /*double cpr_x = HEXAPOD_INIT_FOOT_POS_X[leg_index] + COXA_TO_CENTER_X[leg_index] + feet.foot[leg_index].position.x;
    double cpr_y = (HEXAPOD_INIT_FOOT_POS_Y[leg_index] + COXA_TO_CENTER_Y[leg_index]) + feet.foot[leg_index].position.y;
    double cpr_z = TARSUS_LENGTH - HEXAPOD_INIT_FOOT_POS_Z[leg_index] + feet.foot[leg_index].position.z;*/

    double cpr_x = initFeet.foot[leg_index].position.x + COXA_TO_CENTER_X[leg_index] + feet.foot[leg_index].position.x;
    double cpr_y = (initFeet.foot[leg_index].position.y + COXA_TO_CENTER_Y[leg_index]) + feet.foot[leg_index].position.y;
    double cpr_z = TARSUS_LENGTH - initFeet.foot[leg_index].position.z + feet.foot[leg_index].position.z;
    ...
    // 足端到髋关节的位置x,y,z
    /*double feet_pos_x = HEXAPOD_INIT_FOOT_POS_X[leg_index] + feet.foot[leg_index].position.x + body_pos_x;
    double feet_pos_y = (HEXAPOD_INIT_FOOT_POS_Y[leg_index]) + body_pos_y + feet.foot[leg_index].position.y;
    double feet_pos_z = HEXAPOD_INIT_FOOT_POS_Z[leg_index] - TARSUS_LENGTH - feet.foot[leg_index].position.z;*/

    double feet_pos_x = initFeet.foot[leg_index].position.x + feet.foot[leg_index].position.x + body_pos_x;
    double feet_pos_y = initFeet.foot[leg_index].position.y + body_pos_y + feet.foot[leg_index].position.y;
    double feet_pos_z = initFeet.foot[leg_index].position.z - TARSUS_LENGTH - feet.foot[leg_index].position.z;
...
void Crab_IK::calculateIK(const hexapod_msgs::FeetPositions initFeet, const hexapod_msgs::FeetPositions &feet, hexapod_msgs::LegsJoints *legs)
{
  for (int leg_index = 0; leg_index < NUMBER_OF_LEGS; leg_index++)
  {
    //机体旋转角度，angular.z
    Trig A = getSinCos(feet.foot[leg_index].orientation.yaw);
    //足端到机体中心距离
    /*double cpr_x = CRAB_INIT_FOOT_POS_X[leg_index] + COXA_TO_CENTER_X[leg_index] + feet.foot[leg_index].position.x;
    double cpr_y = (CRAB_INIT_FOOT_POS_Y[leg_index] + COXA_TO_CENTER_Y[leg_index]) + feet.foot[leg_index].position.y;
    double cpr_z = TARSUS_LENGTH - CRAB_INIT_FOOT_POS_Z[leg_index] + feet.foot[leg_index].position.z;*/
    ...
    // 足端到髋关节的位置x,y,z
    /*double feet_pos_x = CRAB_INIT_FOOT_POS_X[leg_index] + feet.foot[leg_index].position.x + body_pos_x;
    double feet_pos_y = (CRAB_INIT_FOOT_POS_Y[leg_index]) + body_pos_y + feet.foot[leg_index].position.y;
    double feet_pos_z = CRAB_INIT_FOOT_POS_Z[leg_index] - TARSUS_LENGTH - feet.foot[leg_index].position.z;*/
